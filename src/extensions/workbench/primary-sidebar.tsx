...
import { workbenchViewContainerRegistry, workbenchViewRegistry } from "./registry";
...
export function PrimarySidebar() {
  ...
  const containers = useRegistrySubscription(
    ...
    () =>
      [...workbenchViewContainerRegistry.get()].sort((a, b) => a.order - b.order),
  );
  ...
  const primaryViews = useMemo(
    () =>
      [...allViews]
        .filter(
          (v) => v.location === "primary" && v.containerId === state.activeActivityId,
        )
        .sort((a, b) => a.order - b.order),
    [allViews, state.activeActivityId],
  );

  const activeContainer = containers.find((c) => c.id === state.activeActivityId);
  ...
  return (
    <div
      className="relative flex min-h-0 shrink-0 flex-col self-stretch border-r border-border bg-muted"
      style={{ width: `${sidebarWidth}px` }}
    >
      <div
        className="flex h-11 shrink-0 items-center gap-0.5 border-b border-border px-1.5 py-1"
        data-tauri-no-drag
      >
        {containers.map((c) => {
          ...
          return (
            <Button
              ...
              aria-label={c.title}
              aria-pressed={active}
              className={cn("shrink-0", active && "bg-muted-foreground/15 text-foreground")}
      ...
      <div className="flex min-h-0 flex-1 flex-col">
        {primaryViews.length === 0 ? (
          <>
            ...
            <div className="text-muted-foreground p-3 text-sm">No views contributed.</div>
          </>
        ) : (
